...
class TestSuccessfulLoginUser(BaseTest):
    ...
    def test_successful_login_user(self):
        ...
        self.assertEqual(user_info["name"], "张三")  # 普通用户显示名
        self.assertEqual(user_info["role"], "普通用户")  # 普通用户角色
        
        # 添加一个故意失败的断言用于测试
        expect(self.page).to_have_url("http://localhost:8080/wrong-url.html")  # 故意错误的URL断言
