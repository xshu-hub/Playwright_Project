...
# 运行指定测试文件
pytest tests/test_practice_page.py

# 运行指定测试用例
pytest tests/test_practice_page.py::TestPracticePage::test_basic_form_submission
...
A: 这个问题已在最新版本中修复：
```python
# 问题原因：Helper类使用了硬编码的默认路径
# 解决方案：现在Helper类会从环境变量获取会话目录

# 如果仍遇到此问题，请检查以下配置：
# 1. 确保使用最新的Helper类实例化方式
# 2. 检查环境变量PYTEST_SESSION_DIR是否正确设置
# 3. 清理旧的空目录：
